...
public class armTrajectory extends CommandBase {
    ...
    public void execute() {
        if (!exception) {
            ...
            JointAngles targetAngles = JointAngles.anglesFrom2D(targetState.poseMeters.getX(),
                    targetState.poseMeters.getY(), finalWristAngle);
            Rotation2d direction = targetState.poseMeters.getRotation();
            Vector<N2> linearVelocity = VecBuilder.fill(direction.getCos(), direction.getSin())
                    .times(targetState.velocityMetersPerSecond);

            targetAngles.shoulderAngle = MathUtil.clamp(targetAngles.shoulderAngle, Math.toRadians(45),
                    Math.toRadians(97));
            ...
            arm.setWrist(
                    MathUtil.clamp(initialWristAngle
                            + (finalWristAngle - initialWristAngle) * timer.get() / trajectory.getTotalTimeSeconds(),
                            -(Math.PI / 2), Math.PI / 2),
